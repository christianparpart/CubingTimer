...
class SqliteSolveStore: public CubingCore::ISolveStore
{
  public:
    ...
    [[nodiscard]] std::expected<CubingCore::Profile, CubingCore::StoreError>
        createProfile(std::string_view name) override;
    [[nodiscard]] std::expected<std::vector<CubingCore::Profile>, CubingCore::StoreError>
        listProfiles() override;
    [[nodiscard]] std::expected<void, CubingCore::StoreError> deleteProfile(std::int64_t profileId) override;

    [[nodiscard]] std::expected<CubingCore::Session, CubingCore::StoreError>
        createSession(std::int64_t profileId, std::string_view name, CubingCore::Puzzle puzzle) override;
    [[nodiscard]] std::expected<std::vector<CubingCore::Session>, CubingCore::StoreError>
        listSessions(std::int64_t profileId) override;
    [[nodiscard]] std::expected<void, CubingCore::StoreError> deleteSession(std::int64_t sessionId) override;

    [[nodiscard]] std::expected<CubingCore::Solve, CubingCore::StoreError>
        addSolve(CubingCore::Solve solve) override;
    [[nodiscard]] std::expected<void, CubingCore::StoreError>
        updateSolve(CubingCore::Solve const& solve) override;
    [[nodiscard]] std::expected<void, CubingCore::StoreError> deleteSolve(std::int64_t solveId) override;
    [[nodiscard]] std::expected<std::vector<CubingCore::Solve>, CubingCore::StoreError>
        loadSession(std::int64_t sessionId) override;
